...
parse_apt :: proc(parser: ^Parser, builder: ^Apt_Builder) -> (err: Apt_Parsing_Error) {
	for {
		...
		case .Colon, .Bang:
			advance_token(parser)
			node := tb_leaf(builder, Apt_Node_Data{kind = .Leaf, tok = tok})
			node.tok.kind = .Word
		case .LCurly:
			advance_token(parser)
			node := tb_leaf(builder, Apt_Node_Data{kind = .Leaf, tok = tok})
		...
		case .Word, .Space, .Newline:
			advance_token(parser)
			tb_leaf(builder, Apt_Node_Data{kind = .Leaf, tok = tok})
		case .Apparition:
			advance_token(parser)
			node := tb_node_begin(builder, Apt_Node_Data{kind = .Node, tok = tok})
...
vapf_iter_next :: proc(iter: ^Varied_Apf_Iter) -> (out: Apt_Old, ok: bool) {
...
apt_parse_def :: proc(eval: Evaluator, spec: Apparition_Spec, node: Apt_Old) -> (ok: bool) {
